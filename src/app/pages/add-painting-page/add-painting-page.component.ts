...
@Component({
  ...
  styleUrl: './add-painting-page.component.css'
})

export class AddPaintingPageComponent {
  ...
  async onSubmit() {
    this.artwork.category = this.selectedCategories; 
    this.artwork.id = Date.now().toString();
    await this.paintingService.setItem(this.artwork);
    console.log('Artwork added:', this.artwork);
    this.clearForm();
